...
import { K0sInstallStatus, HostPreflightStatus } from '../types';
import { installK0s } from '../utils/k0s';
import { validateHostPreflights } from '../utils/validation';

export type NodeRole = 'application' | 'database';
...
export const useK0sInstallation = (onComplete: (hasFailures?: boolean) => void) => {
  ...
  const startPreflightChecks = async () => {
    try {
      ...
      setHasPreflightFailures(hasErrors);
  ...
  const startInstallation = async () => {
    try {
      ...
      setNodeMetrics(prev => ({
        ...prev,
        application: {
          'wordpress-app-1': baseNodeMetrics
        }
      }));
    } catch (error) {
  ...
  const handleNodeJoined = () => {
    ...
    const nodeName = selectedRole === 'application' ? 
      `wordpress-app-${nodeNumber}` : 
      `wordpress-db-${nodeNumber}`;
